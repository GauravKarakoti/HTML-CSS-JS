document.addEventListener("DOMContentLoaded", () => {
    ...
    cashInputs.forEach(input => {
        input.addEventListener("input", () => {
            const value = parseInt(input.value, 9);
    ...
    function convertToWords(number) {
        ...
        if(Math.floor(number / 1000000) > 0) {
            words += convertToWords(Math.floor(number / 1000000)) + " Crore ";
            number %= 1000000;
        }
        ...
        if(Math.floor(number / 100) > 0) {
            words += convertToWords(Math.floor(number / 100)) + " Hundred ";    // 1 --- One Hundred
            number %= 100;
        }
        if(number > 0) {
            if(number < 10) {
                ...
                // 25 ---> 20  5 -- Twenty Five
                words += tens[Math.floor(number / 10)];    // 2
                if(number % 10 > 0) {
